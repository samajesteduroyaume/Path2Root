...
export const Register: React.FC<{ onToggle: () => void }> = ({ onToggle }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setError('');
        try {
            const resp = await axios.post('/api/auth/register', { username, password });
        ...
        <div className="min-h-screen w-full flex items-center justify-center bg-[#0d0d0d] relative overflow-hidden">
            ...
            <div className="w-full max-w-md p-8 glass-panel rounded-3xl border border-white/10 shadow-2xl relative z-10 m-4">
                <div className="flex flex-col items-center mb-10">
                    <div className="p-4 bg-emerald-500/20 rounded-2xl mb-4 border border-emerald-500/20 shadow-[0_0_20px_rgba(16,185,129,0.2)]">
                        <Sparkles className="text-emerald-400" size={40} />
                    </div>
                    <h1 className="text-3xl font-black text-white tracking-tight">New Operator</h1>
                    <p className="text-slate-500 text-sm mt-2">Initialize your security profile</p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div className="space-y-2">
                        ...
                        <div className="relative">
                            ...
                            <input
                                ...
                                className="w-full bg-slate-900/50 border border-slate-800 rounded-xl py-4 pl-12 pr-4 focus:ring-2 focus:ring-emerald-500 outline-none transition-all text-white placeholder:text-slate-600"
                                placeholder="ID-XXXX-OP"
                                required
                            />
                        </div>
                    </div>
                    ...
                    <button
                        type="submit"
                        disabled={isLoading}
                        className="w-full bg-emerald-600 hover:bg-emerald-500 disabled:bg-slate-800 text-white font-black py-4 rounded-xl transition-all shadow-xl shadow-emerald-600/20 flex items-center justify-center gap-2 group"
                    >
                ...
                <div className="mt-8 pt-8 border-t border-white/5 text-center">
                    ...
                    >
                        Already have an account? Portal access
                    </button>
                </div>
            </div>
        </div>
